...
const CHECKOUT_ANALYTICS_HEADERS = ['StoreKey','SessionID','Event','Timestamp','Stage','Wilaya','DeliveryType','Field','CartCount','CartValue','Total','Path'];
...
function getCheckoutAnalyticsSheet_() {
  ...
  if (sh.getLastRow() === 0) {
    ...
    const current = sh.getRange(1,1,1,Math.max(sh.getLastColumn(),CHECKOUT_ANALYTICS_HEADERS.length)).getValues()[0].map(String);
    if (current.slice(0,CHECKOUT_ANALYTICS_HEADERS.length).join('|') !== CHECKOUT_ANALYTICS_HEADERS.join('|')) {
      sh.getRange(1,1,1,CHECKOUT_ANALYTICS_HEADERS.length).setValues([CHECKOUT_ANALYTICS_HEADERS]);
    }
  }
  return sh;
...
function recordCheckoutEvent(data) {
  const storeKey = normalizeStoreKey(data.storeKey);
  const d = data.data || {};
  const allowed = ['begin_checkout','wilaya_selected','delivery_selected','details_started','checkout_submit_attempt','purchase','checkout_abandoned'];
  ...
  try {
    sh.appendRow([
      storeKey, session, event, String(d.timestamp || new Date().toISOString()), Number(d.stage||0),
      String(d.wilaya||''), String(d.deliveryType||''), String(d.field||''), Number(d.cartCount||0),
      Number(d.cartValue||0), Number(d.total||0), String(d.path||'')
    ]);
...
function checkoutAnalytics(body) {
  ...
  rows.forEach(r=>{
    ...
    events.push({storeKey:String(r[0]||''),sessionId:String(r[1]||''),event:String(r[2]||''),timestamp:isFinite(dt.getTime())?dt.toISOString():String(r[3]||''),stage:Number(r[4]||0),wilaya:String(r[5]||''),deliveryType:String(r[6]||''),field:String(r[7]||''),cartCount:Number(r[8]||0),cartValue:Number(r[9]||0),total:Number(r[10]||0),path:String(r[11]||'')});
  });
  return buildCheckoutAnalyticsResult_(events, storeKey);
}
function buildCheckoutAnalyticsResult_(events, storeKey) {
  const sessions = {};
  events.forEach(e=>{if(!sessions[e.sessionId]) sessions[e.sessionId]={events:[],begin:false,purchase:false,abandoned:false,lastStage:0,wilaya:'',deliveryType:'',total:0}; const s=sessions[e.sessionId]; s.events.push(e); if(e.event==='begin_checkout')s.begin=true; if(e.event==='purchase'){s.purchase=true;s.total=e.total||s.total;} if(e.event==='checkout_abandoned')s.abandoned=true; if(e.stage)s.lastStage=Math.max(s.lastStage,e.stage); if(e.wilaya)s.wilaya=e.wilaya; if(e.deliveryType)s.deliveryType=e.deliveryType;});
  const ids=Object.keys(sessions).filter(id=>sessions[id].begin);
  const uniqueCount=(event)=>ids.filter(id=>sessions[id].events.some(e=>e.event===event)).length;
  const begin=ids.length, wilaya=uniqueCount('wilaya_selected'), delivery=uniqueCount('delivery_selected'), details=uniqueCount('details_started'), attempts=uniqueCount('checkout_submit_attempt'), purchases=ids.filter(id=>sessions[id].purchase).length;
  const abandoned=ids.filter(id=>!sessions[id].purchase).length;
  const explicitAbandoned=ids.filter(id=>sessions[id].abandoned&&!sessions[id].purchase).length;
  const pct=(a,b)=>b?Math.round(a/b*1000)/10:0;
  const byWilaya={}; const byDelivery={};
  ids.forEach(id=>{const s=sessions[id]; if(s.wilaya){if(!byWilaya[s.wilaya]) byWilaya[s.wilaya]={sessions:0,purchases:0};byWilaya[s.wilaya].sessions++;if(s.purchase)byWilaya[s.wilaya].purchases++;} if(s.deliveryType){if(!byDelivery[s.deliveryType]) byDelivery[s.deliveryType]={sessions:0,purchases:0};byDelivery[s.deliveryType].sessions++;if(s.purchase)byDelivery[s.deliveryType].purchases++;}});
  const abandonedSessions=ids.filter(id=>!sessions[id].purchase).map(id=>{const s=sessions[id]; const last=s.events[s.events.length-1]||{}; return {sessionId:id,lastStage:s.lastStage||last.stage||1,wilaya:s.wilaya||'',deliveryType:s.deliveryType||'',lastEvent:last.event||'',lastAt:last.timestamp||'',cartValue:last.cartValue||0,total:last.total||s.total||0};}).sort((a,b)=>String(b.lastAt).localeCompare(String(a.lastAt))).slice(0,100);
  return {success:true,storeKey,generatedAt:new Date().toISOString(),summary:{sessions:begin,stage1:begin,stage2:wilaya,stage3:delivery,stage4:details,submitAttempts:attempts,purchases:purchases,conversionRate:pct(purchases,begin),abandoned:abandoned,abandonedRate:pct(abandoned,begin),explicitAbandoned:explicitAbandoned},funnel:[{stage:1,label:'السلة / بدء Checkout',sessions:begin,rate:100},{stage:2,label:'اختيار الولاية',sessions:wilaya,rate:pct(wilaya,begin)},{stage:3,label:'طريقة التوصيل',sessions:delivery,rate:pct(delivery,begin)},{stage:4,label:'بيانات العميل',sessions:details,rate:pct(details,begin)},{stage:5,label:'محاولة الطلب',sessions:attempts,rate:pct(attempts,begin)},{stage:6,label:'Purchase',sessions:purchases,rate:pct(purchases,begin)}],byWilaya:Object.entries(byWilaya).map(([name,v])=>({name,...v,conversionRate:pct(v.purchases,v.sessions)})).sort((a,b)=>b.sessions-a.sessions),byDelivery:Object.entries(byDelivery).map(([name,v])=>({name,...v,conversionRate:pct(v.purchases,v.sessions)})).sort((a,b)=>b.sessions-a.sessions),abandonedSessions};
}
