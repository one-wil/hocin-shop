...
// بنية ورقة الطلبات: عنوان في الصف 1، رؤوس في الصف 2، والبيانات تبدأ من الصف 3.
const TITLE_ROW = 1;
const HEADER_ROW = 2;
const DATA_START_ROW = 3;
const ARCHIVE_SHEET_NAME = 'Archive_Commandes';
const ARCHIVE_HEADER_ROW = 2;
const ARCHIVE_DATA_START_ROW = 3;
const ACTIVE_DAYS = 30;
const FINAL_RETENTION_DAYS = 90;
...
const SHEET_HEADERS = [
  ...
  'Sous-total','Remise','Frais Livraison','Total Général','OrderData','UpdatedAt'
];
...
function ensureOrderSheetSchema(sheet) {
  ...
  // V8.3: technical modification timestamp used by the smart client cache.
  let headersAfterOrderData = sheet.getRange(HEADER_ROW, 1, 1, Math.max(sheet.getLastColumn(), SHEET_WIDTH)).getValues()[0].map(v => String(v || '').trim());
  if (headersAfterOrderData.indexOf('UpdatedAt') === -1) {
    sheet.insertColumnAfter(sheet.getLastColumn());
  }
  // Normalize expected headers/positions.
  SHEET_HEADERS.forEach((h, i) => sheet.getRange(HEADER_ROW, i + 1).setValue(h));

  // Backfill legacy rows once: UpdatedAt falls back to the original order date.
  try {
    const lastData = sheet.getLastRow();
    const updCol = SHEET_HEADERS.indexOf('UpdatedAt') + 1;
    const dateCol = SHEET_HEADERS.indexOf('Date et Heure') + 1;
    if (lastData >= DATA_START_ROW && updCol > 0 && dateCol > 0) {
      const upd = sheet.getRange(DATA_START_ROW, updCol, lastData - DATA_START_ROW + 1, 1).getValues();
      const dates = sheet.getRange(DATA_START_ROW, dateCol, lastData - DATA_START_ROW + 1, 1).getValues();
      let dirty = false;
      for (let i=0;i<upd.length;i++) { if (!upd[i][0] && dates[i][0]) { upd[i][0] = dates[i][0]; dirty = true; } }
      if (dirty) sheet.getRange(DATA_START_ROW, updCol, upd.length, 1).setValues(upd);
    }
  } catch (e) { console.warn('UpdatedAt backfill:', e); }
...
function setColumnWidths(sheet) {
  const widths = {1:150,2:150,3:145,4:145,5:155,6:150,7:125,8:125,9:190,10:110,11:140,12:390,13:105,14:120,15:110,16:125,17:130,18:40,19:145};
...
function getOrdersSheet() {
  ...
  // استعمال الورقة الموجودة حتى لو كان اسمها "commandes" بحروف صغيرة.
  let sheet = ss.getSheetByName(SHEET_NAME);

  if (!sheet) {
    const wanted = String(SHEET_NAME).trim().toLowerCase();
    const sheets = ss.getSheets();
    for (let i = 0; i < sheets.length; i++) {
      if (String(sheets[i].getName()).trim().toLowerCase() === wanted) {
        sheet = sheets[i];
        break;
      }
    }
...
// =====================================================================
// ARCHIVAGE AUTOMATIQUE — 30 jours actifs / 90 jours rétention totale
// =====================================================================
function getArchiveSheet() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(ARCHIVE_SHEET_NAME);
  if (!sheet) sheet = ss.insertSheet(ARCHIVE_SHEET_NAME);
  ensureArchiveSheetSchema(sheet);
  return sheet;
}

function ensureArchiveSheetSchema(sheet) {
  const headers = SHEET_HEADERS.concat(['ArchivedAt']);
  if (sheet.getLastRow() === 0) {
    sheet.getRange(1, 1, 1, headers.length).merge();
    sheet.getRange(1, 1).setValue('📦 SHOPLIVE — ARCHIVE DES COMMANDES')
      .setFontSize(18).setFontWeight('bold').setFontColor('#FFFFFF').setBackground('#674EA7')
      .setHorizontalAlignment('center');
    sheet.setRowHeight(1, 44);
    sheet.getRange(2, 1, 1, headers.length).setValues([headers]);
    sheet.getRange(2, 1, 1, headers.length).setBackground('#8E7CC3').setFontColor('#FFFFFF')
      .setFontWeight('bold').setHorizontalAlignment('center').setWrap(true);
    sheet.setFrozenRows(2);
    const widths = [150,150,145,145,155,150,125,125,190,110,140,390,105,120,110,125,130,40,145,145];
    widths.forEach((w,i)=>sheet.setColumnWidth(i+1,w));
  } else {
    const current = sheet.getRange(2,1,1,Math.max(sheet.getLastColumn(),headers.length)).getValues()[0].map(v=>String(v||'').trim());
    headers.forEach((h,i)=>sheet.getRange(2,i+1).setValue(h));
    if (current[19] !== 'ArchivedAt') sheet.getRange(2,20).setValue('ArchivedAt');
    try { sheet.getRange(1,1,1,sheet.getMaxColumns()).breakApart(); } catch (_) {}
    try { sheet.getRange(1,1,1,headers.length).merge(); } catch (_) {}
    sheet.getRange(1,1).setValue('📦 SHOPLIVE — ARCHIVE DES COMMANDES')
      .setFontSize(18).setFontWeight('bold').setFontColor('#FFFFFF').setBackground('#674EA7')
      .setHorizontalAlignment('center');
    sheet.setFrozenRows(2);
...
function archiveAgeDays(date) {
  if (!date) return null;
  return Math.floor((Date.now() - date.getTime()) / 86400000);
}

function archiveOrderRowExists(archiveSheet, orderId, storeKey) {
  if (!orderId) return false;
  const last = archiveSheet.getLastRow();
  if (last < ARCHIVE_DATA_START_ROW) return false;
  const values = archiveSheet.getRange(ARCHIVE_DATA_START_ROW,1,last-ARCHIVE_DATA_START_ROW+1,Math.max(archiveSheet.getLastColumn(),20)).getValues();
  return values.some(r => String(r[4]||'').trim() === String(orderId).trim() && normalizeStoreKey(r[0]) === normalizeStoreKey(storeKey));
}

function archiveOrdersDaily() {
  const lock = LockService.getScriptLock();
  lock.waitLock(30000);
  try {
    const active = getOrdersSheet();
    const archive = getArchiveSheet();
    const idx = getHeaderIndexes(active);
    const last = active.getLastRow();
    let moved = 0, deletedActive = 0, deletedArchive = 0;

    if (last >= DATA_START_ROW) {
      const width = Math.max(active.getLastColumn(), SHEET_WIDTH);
      const values = active.getRange(DATA_START_ROW,1,last-DATA_START_ROW+1,width).getValues();
      const rowsToDelete = [];
      const archiveRows = [];
      const now = new Date();
      // تحميل مفاتيح الأرشيف مرة واحدة بدلاً من البحث داخل الورقة لكل طلب.
      const archiveKeys = new Set();
      const archiveLastForKeys = archive.getLastRow();
      if (archiveLastForKeys >= ARCHIVE_DATA_START_ROW) {
        const keyValues = archive.getRange(ARCHIVE_DATA_START_ROW,1,archiveLastForKeys-ARCHIVE_DATA_START_ROW+1,5).getValues();
        keyValues.forEach(r => {
          const sk=normalizeStoreKey(r[0]), oid=String(r[4]||'').trim();
          if (sk && oid) archiveKeys.add(sk+'|'+oid);
        });
      }

      values.forEach((row, offset) => {
        const orderDate = parseArchiveDate(row[idx['Date et Heure']]);
        const age = archiveAgeDays(orderDate);
        if (age === null || age < ACTIVE_DAYS) return;
        const orderId = String(row[idx['ID Commande']] || '').trim();
        const storeKey = normalizeStoreKey(row[idx.StoreKey]);
        const rowNumber = DATA_START_ROW + offset;

        // Au-delà de 90 jours, le système supprime définitivement l'enregistrement actif.
        if (age >= FINAL_RETENTION_DAYS) {
          rowsToDelete.push(rowNumber);
          deletedActive++;
          return;
        }

        const archiveKey = storeKey + '|' + orderId;
        if (!orderId || archiveKeys.has(archiveKey)) {
          rowsToDelete.push(rowNumber);
          if (orderId) moved++;
          return;
        }

        const out = new Array(20).fill('');
        for (let i=0;i<SHEET_WIDTH;i++) out[i] = row[i] === undefined ? '' : row[i];
        out[19] = now;
        archiveRows.push(out);
      ...
      if (archiveRows.length) {
        const start = Math.max(archive.getLastRow()+1, ARCHIVE_DATA_START_ROW);
        archive.getRange(start,1,archiveRows.length,20).setValues(archiveRows);
        archive.getRange(start,4,archiveRows.length,1).setNumberFormat('dd/MM/yyyy HH:mm');
        archive.getRange(start,20,archiveRows.length,1).setNumberFormat('dd/MM/yyyy HH:mm');
      }

      // Suppression en remontant pour ne pas décaler les lignes.
      rowsToDelete.sort((a,b)=>b-a).forEach(r=>active.deleteRow(r));
    }

    // Purge définitive de l'archive au-delà de 90 jours depuis la date originale de la commande.
    const alast = archive.getLastRow();
    if (alast >= ARCHIVE_DATA_START_ROW) {
      const avalues = archive.getRange(ARCHIVE_DATA_START_ROW,1,alast-ARCHIVE_DATA_START_ROW+1,20).getValues();
      const toDelete = [];
      const dateIdx = SHEET_HEADERS.indexOf('Date et Heure');
      avalues.forEach((row,i)=>{
        const d = parseArchiveDate(row[dateIdx]);
        const age = archiveAgeDays(d);
        if (age !== null && age >= FINAL_RETENTION_DAYS) toDelete.push(ARCHIVE_DATA_START_ROW+i);
      });
      toDelete.sort((a,b)=>b-a).forEach(r=>archive.deleteRow(r));
      deletedArchive = toDelete.length;
    }

    return {success:true,moved,deletedActive,deletedArchive,activeDays:ACTIVE_DAYS,finalRetentionDays:FINAL_RETENTION_DAYS};
  } finally {
...
function setupArchiveSystem() {
  ...
  triggers.forEach(t => { if (t.getHandlerFunction() === 'archiveOrdersDaily') ScriptApp.deleteTrigger(t); });
  ScriptApp.newTrigger('archiveOrdersDaily').timeBased().everyDays(1).atHour(2).create();
  return {success:true,message:'Archivage automatique configuré: 30 jours actifs, suppression définitive à 90 jours.'};
}

function apiListArchive(body) {
  ...
  const matches = [];
  if (last >= ARCHIVE_DATA_START_ROW) {
    const values = sheet.getRange(ARCHIVE_DATA_START_ROW,1,last-ARCHIVE_DATA_START_ROW+1,19).getValues();
    values.forEach((row,n)=>{
      if (normalizeStoreKey(row[0]) !== storeKey) return;
      const idx = getHeaderIndexesForHeaders(SHEET_HEADERS.concat(['ArchivedAt']));
      const order = rowToOrder(row, idx, n + ARCHIVE_DATA_START_ROW);
      if (matchesOrderFilters(order, filters)) matches.push(order);
    });
  }
  matches.sort((a,b)=>(parseOrderDate(b.timestamp)?.getTime()||0)-(parseOrderDate(a.timestamp)?.getTime()||0));
  const total = matches.length;
  const start=(page-1)*pageSize;
  return {success:true,orders:matches.slice(start,start+pageSize),total,page,pageSize,totalPages:Math.max(1,Math.ceil(total/pageSize))};
}

function getHeaderIndexesForHeaders(headers) {
  const idx={};
  headers.forEach((h,i)=>idx[h]=i);
  return idx;
}

function apiExportArchive(body) {
  const storeKey = normalizeStoreKey(body.storeKey);
  if (!storeKey) throw new Error('StoreKey obligatoire');
  const sheet = getArchiveSheet();
  const last = sheet.getLastRow();
  ...
  if (last >= ARCHIVE_DATA_START_ROW) {
    ...
    values.forEach((row,n)=>{
      if (normalizeStoreKey(row[0]) !== storeKey) return;
      const order=rowToOrder(row,idx,n+ARCHIVE_DATA_START_ROW);
      if (matchesOrderFilters(order,filters)) rows.push(row);
    });
  }
  const headers = SHEET_HEADERS.concat(['ArchivedAt']);
  const escCsv = v => '"' + String(v instanceof Date ? Utilities.formatDate(v, Session.getScriptTimeZone(), 'dd/MM/yyyy HH:mm') : (v ?? '')).replace(/"/g,'""') + '"';
  const csv = [headers.map(escCsv).join(';')].concat(rows.map(r=>r.map(escCsv).join(';'))).join('\r\n');
  return {success:true,filename:'Archive_Commandes_'+Utilities.formatDate(new Date(),Session.getScriptTimeZone(),'yyyyMMdd_HHmm')+'.csv',csv,count:rows.length};
}
...
function apiListOrders(body) {
  ...
  // الطلبات النشطة فقط: آخر 30 يوماً، ما لم يحدد المدير فترة يدوياً.
  const filters = Object.assign({}, body.filters || {});
  const activeFrom = new Date();
  activeFrom.setHours(0,0,0,0);
  activeFrom.setDate(activeFrom.getDate() - ACTIVE_DAYS);
  const activeFromText = Utilities.formatDate(activeFrom, Session.getScriptTimeZone(), 'yyyy-MM-dd');
  const todayText = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  // حتى مع فلترة يدوية لا نسمح لقسم الطلبات النشطة بعرض ما قبل 30 يوماً.
  if (!filters.dateFrom || String(filters.dateFrom) < activeFromText) filters.dateFrom = activeFromText;
  if (!filters.dateTo || String(filters.dateTo) > todayText) filters.dateTo = todayText;
  ...
  if (lastRow >= DATA_START_ROW) {
    ...
    values.forEach((row, n) => {
      ...
      const order = rowToOrder(row, idx, n + DATA_START_ROW);
      if (matchesOrderFilters(order, filters)) orders.push(order);
    });
  }

  orders.sort((a, b) =>
    (parseOrderDate(b.timestamp)?.getTime() || 0) -
    (parseOrderDate(a.timestamp)?.getTime() || 0)
  );
...
function apiSyncOrders(body) {
  ...
  if (lastRow >= DATA_START_ROW) {
    ...
    values.forEach((row,n)=>{
      if (normalizeStoreKey(row[idx.StoreKey]) !== storeKey) return;
      const updated = idx.UpdatedAt >= 0 && row[idx.UpdatedAt] instanceof Date ? row[idx.UpdatedAt] : row[idx['Date et Heure']];
      if (!(updated instanceof Date)) return;
      if (updated.getTime() > sinceDate.getTime()) changed.push(rowToOrder(row,idx,n+DATA_START_ROW));
    });
  }
  changed.sort((a,b)=>(new Date(b.updatedAt||b.timestamp).getTime()||0)-(new Date(a.updatedAt||a.timestamp).getTime()||0));
  const serverTime = new Date().toISOString();
  return {success:true,orders:changed.slice(0,limit),hasMore:changed.length>limit,serverTime,since};
}
...
function apiGetOrder(body) {
  ...
  for (let i=0;i<values.length;i++) {
    if (normalizeStoreKey(values[i][idx.StoreKey]) === storeKey && String(values[i][idx['ID Commande']] || '') === orderId) return { success:true, order:rowToOrder(values[i],idx,i+DATA_START_ROW) };
  }
...
function apiUpdateOrderStatus(body) {
  ...
  for (let i=0;i<values.length;i++) {
    if (normalizeStoreKey(values[i][idx.StoreKey]) === storeKey && String(values[i][idx['ID Commande']] || '') === orderId) {
      ...
      if (idx.UpdatedAt >= 0) sheet.getRange(rowNumber, idx.UpdatedAt + 1).setValue(new Date());
...
function rowToOrder(row, idx, rowNumber) {
  ...
  return {
    ...
    updatedAt: row[idx.UpdatedAt] instanceof Date ? row[idx.UpdatedAt].toISOString() : (row[idx.UpdatedAt] ? String(row[idx.UpdatedAt]) : '')
  };
}
...
// ---------- Checkout Analytics المركزي ----------
const CHECKOUT_ANALYTICS_SHEET = 'Checkout_Analytics';
const CHECKOUT_ANALYTICS_HEADERS = ['StoreKey','SessionID','Event','Timestamp','Stage','Wilaya','DeliveryType','Field','CartCount','CartValue','Total','Path','Items'];

function getCheckoutAnalyticsSheet_() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sh = ss.getSheetByName(CHECKOUT_ANALYTICS_SHEET);
  if (!sh) sh = ss.insertSheet(CHECKOUT_ANALYTICS_SHEET);
  if (sh.getLastRow() === 0) {
    sh.getRange(1,1,1,CHECKOUT_ANALYTICS_HEADERS.length).setValues([CHECKOUT_ANALYTICS_HEADERS]);
    sh.setFrozenRows(1);
  } else {
    // Migration sûre : ne remplace jamais les anciens enregistrements.
    // On ajoute seulement les colonnes manquantes (notamment Items).
    const lastColumn = Math.max(sh.getLastColumn(), 1);
    const current = sh.getRange(1,1,1,lastColumn).getValues()[0].map(v => String(v || '').trim());
    CHECKOUT_ANALYTICS_HEADERS.forEach((header) => {
...
function recordCheckoutEvent(data) {
  const storeKey = normalizeStoreKey(data.storeKey);
  const d = data.data || {};
  const allowed = ['store_home_view','product_view','begin_checkout','wilaya_selected','delivery_selected','details_started','checkout_submit_attempt','purchase','checkout_abandoned'];
  const event = String(d.event || '').trim();
  const session = String(d.sessionId || '').trim();
  if (!storeKey || !session || allowed.indexOf(event) < 0) return {success:false,error:'Données Analytics invalides'};
  const sh = getCheckoutAnalyticsSheet_();
  const lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    const items = sanitizeCheckoutItems_(d.items);
    const itemJson = items.length ? JSON.stringify(items) : '';
    const row = new Array(Math.max(sh.getLastColumn(), CHECKOUT_ANALYTICS_HEADERS.length)).fill('');
    row[0] = storeKey;
    row[1] = session;
    row[2] = event;
    row[3] = String(d.timestamp || new Date().toISOString());
    row[4] = Number(d.stage||0);
    row[5] = String(d.wilaya||'');
    row[6] = String(d.deliveryType||'');
    row[7] = String(d.field||'');
    row[8] = Number(d.cartCount||0);
    row[9] = Number(d.cartValue||0);
    row[10] = Number(d.total||0);
    row[11] = String(d.path||'');
    const itemsIndex = CHECKOUT_ANALYTICS_HEADERS.indexOf('Items');
    if (itemsIndex >= 0) row[itemsIndex] = itemJson;
    sh.appendRow(row);
    return {success:true,event:event,sessionId:session};
  } finally {
    lock.releaseLock();
  }
}
function sanitizeCheckoutItems_(items) {
  if (!Array.isArray(items)) return [];
  return items.slice(0,30).map(function(it) {
    it = it || {};
    return {
      name: String(it.name || it.productName || it.title || 'منتج').slice(0,160),
      quantity: Math.max(1, Math.min(999, Number(it.quantity) || 1)),
      price: Number(it.price) || 0,
      finalPrice: Number(it.finalPrice != null ? it.finalPrice : it.price) || 0,
...
function parseCheckoutItems_(value) {
  if (!value) return [];
  try {
    const parsed = typeof value === 'string' ? JSON.parse(value) : value;
    return sanitizeCheckoutItems_(parsed);
  } catch (_) { return []; }
}
function checkoutAnalytics(body) {
  const storeKey = normalizeStoreKey(body.storeKey);
  if (!storeKey) return {success:false,error:'StoreKey obligatoire'};
  const f = body.filters || {};
  const from = f.dateFrom ? new Date(String(f.dateFrom)+'T00:00:00') : null;
  const to = f.dateTo ? new Date(String(f.dateTo)+'T23:59:59.999') : null;
  const sh = getCheckoutAnalyticsSheet_();
  const last = sh.getLastRow();
  if (last < 2) return buildCheckoutAnalyticsResult_([], storeKey);
  const rows = sh.getRange(2,1,last-1,CHECKOUT_ANALYTICS_HEADERS.length).getValues();
  const events = [];
  rows.forEach(r=>{
    if (normalizeStoreKey(r[0]) !== storeKey) return;
    const dt = new Date(r[3]);
    if (from && (!isFinite(dt.getTime()) || dt < from)) return;
    if (to && (!isFinite(dt.getTime()) || dt > to)) return;
    events.push({storeKey:String(r[0]||''),sessionId:String(r[1]||''),event:String(r[2]||''),timestamp:isFinite(dt.getTime())?dt.toISOString():String(r[3]||''),stage:Number(r[4]||0),wilaya:String(r[5]||''),deliveryType:String(r[6]||''),field:String(r[7]||''),cartCount:Number(r[8]||0),cartValue:Number(r[9]||0),total:Number(r[10]||0),path:String(r[11]||''),items:parseCheckoutItems_(r[12])});
  });
  return buildCheckoutAnalyticsResult_(events, storeKey);
}
function buildCheckoutAnalyticsResult_(events, storeKey) {
  const stageForEvent_ = (event, fallback) => {
    const map = {
      store_home_view: 1,
      product_view: 2,
      begin_checkout: 3,
      ...
      delivery_selected: 5,
      details_started: 6,
      checkout_submit_attempt: 7,
      purchase: 8,
      checkout_abandoned: Number(fallback || 3)
    };
    return map[event] || Number(fallback || 0);
  };

  const sessions = {};
  events.forEach(e=>{
    e.stage = stageForEvent_(e.event, e.stage);
    if(!sessions[e.sessionId]) sessions[e.sessionId]={events:[],home:false,product:false,begin:false,purchase:false,abandoned:false,lastStage:0,wilaya:'',deliveryType:'',total:0,items:[]};
    const s=sessions[e.sessionId];
    s.events.push(e);
    if(e.event==='store_home_view')s.home=true;
    if(e.event==='product_view')s.product=true;
    if(e.event==='begin_checkout')s.begin=true;
    if(e.event==='purchase'){s.purchase=true;s.total=e.total||s.total;}
    if(e.event==='checkout_abandoned')s.abandoned=true;
    if(e.stage)s.lastStage=Math.max(s.lastStage,e.stage);
    if(e.wilaya)s.wilaya=e.wilaya;
    if(e.deliveryType)s.deliveryType=e.deliveryType;
    if(Array.isArray(e.items)&&e.items.length)s.items=e.items;
  });

  const ids=Object.keys(sessions);
  const checkoutIds=ids.filter(id=>sessions[id].begin);
  const uniqueCount=(event, pool=checkoutIds)=>pool.filter(id=>sessions[id].events.some(e=>e.event===event)).length;
  const visitors=ids.length;
  const home=uniqueCount('store_home_view',ids);
  const product=uniqueCount('product_view',ids);
  const begin=checkoutIds.length;
  const wilaya=uniqueCount('wilaya_selected');
  const delivery=uniqueCount('delivery_selected');
  const details=uniqueCount('details_started');
  const attempts=uniqueCount('checkout_submit_attempt');
  const purchases=checkoutIds.filter(id=>sessions[id].purchase).length;
  const abandoned=checkoutIds.filter(id=>!sessions[id].purchase).length;
  const explicitAbandoned=checkoutIds.filter(id=>sessions[id].abandoned&&!sessions[id].purchase).length;
  const pct=(a,b)=>b?Math.round(a/b*1000)/10:0;
  const byWilaya={}; const byDelivery={};
  checkoutIds.forEach(id=>{
    const s=sessions[id];
    if(s.wilaya){if(!byWilaya[s.wilaya]) byWilaya[s.wilaya]={sessions:0,purchases:0};byWilaya[s.wilaya].sessions++;if(s.purchase)byWilaya[s.wilaya].purchases++;}
    if(s.deliveryType){if(!byDelivery[s.deliveryType]) byDelivery[s.deliveryType]={sessions:0,purchases:0};byDelivery[s.deliveryType].sessions++;if(s.purchase)byDelivery[s.deliveryType].purchases++;}
  });
  const abandonedSessions=checkoutIds.filter(id=>!sessions[id].purchase).map(id=>{
    const s=sessions[id]; const last=s.events[s.events.length-1]||{};
    return {sessionId:id,lastStage:s.lastStage||last.stage||3,wilaya:s.wilaya||'',deliveryType:s.deliveryType||'',lastEvent:last.event||'',lastAt:last.timestamp||'',cartCount:last.cartCount||0,cartValue:last.cartValue||0,total:last.total||s.total||0,items:s.items||last.items||[]};
  ...
  return {
    ...
    summary:{visitors:visitors,sessions:begin,checkoutSessions:begin,stage1:home,stage2:product,stage3:begin,stage4:wilaya,stage5:delivery,stage6:details,submitAttempts:attempts,purchases:purchases,conversionRate:pct(purchases,begin),abandoned:abandoned,abandonedRate:pct(abandoned,begin),explicitAbandoned:explicitAbandoned},
    funnel:[
...
function ordersApi(body) {
  ...
  if (action==='recordCheckoutEvent') return recordCheckoutEvent(body);
  if (action==='checkoutAnalytics') return checkoutAnalytics(body);
  ...
  if (action==='syncOrders') return apiSyncOrders(body);
  if (action==='listArchive') return apiListArchive(body);
  if (action==='exportArchive') return apiExportArchive(body);
  if (action==='setupArchive') return setupArchiveSystem();
  if (action==='runArchiveNow') return archiveOrdersDaily();
